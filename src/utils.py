"""Funzioni di utilità per visualizzazione e analisi dei risultati."""

import matplotlib
matplotlib.use('Agg')  # backend non-interattivo per SLURM
...
def plot_class_distribution(df, area_name='', save_path=None):
    """Plotta la distribuzione delle classi con bar chart e pie chart."""
    ...
    axes[0].set_xlabel('Numero di punti')
    axes[0].set_title(f'Distribuzione delle classi {area_name}')
    ...
    print(f"Classe più frequente: {class_counts.idxmax()} ({class_counts.max():,})")
    print(f"Classe meno frequente: {class_counts.idxmin()} ({class_counts.min():,})")
    print(f"Rapporto max/min: {class_counts.max() / class_counts.min():.1f}x")


def plot_pointcloud_3d(df, color_by='label', max_points=100000, save_path=None):
    """Visualizzazione 3D della point cloud."""
    ...
    if color_by == 'label':
        ...
        ax.set_title('Point Cloud - Colorato per classe')
    elif color_by == 'rgb':
        ...
        ax.set_title('Point Cloud - Colore RGB')
...
def plot_birdseye_view(df, max_points=100000, save_path=None):
    """Vista dall'alto: RGB vs classi semantiche."""
    if len(df) > max_points:
    ...
    axes[0].set_title("Vista dall'alto - RGB")
    axes[0].set_xlabel('X')
    axes[0].set_ylabel('Y')
    ...
    axes[1].set_title("Vista dall'alto - Classi semantiche")
    axes[1].set_xlabel('X')
    axes[1].set_ylabel('Y')
...
def plot_feature_analysis(df, max_points=50000, save_path=None):
    """Box plot delle features per classe + matrice di correlazione."""
    if len(df) > max_points:
        idx = np.random.choice(len(df), max_points, replace=False)
    ...
    for i, feat in enumerate(features):
        ...
        axes[i].set_title(f'{feat} per classe')
...
def plot_correlation_matrix(df, save_path=None):
    """Matrice di correlazione delle features numeriche."""
...
def plot_iou_per_class(ious, miou, save_path=None):
    ...
    plt.ylabel('IoU')
    plt.title('IoU per classe sul test set')
    plt.xticks(rotation=45, ha='right')
    plt.legend()
    plt.tight_layout()
    if save_path:
...
def plot_cv_learning_curves(cv_results, save_path=None):
    """Learning curves per ogni fold della cross-validation."""
    k = len(cv_results)
...
def plot_prediction_comparison(points_np, true_labels, pred_labels, save_path=None):
    """Confronto visivo ground truth vs predizione su un blocco."""
    cmap = plt.cm.get_cmap('tab10', NUM_CLASSES)
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))

    for ax, labels, title in zip(
        axes, [true_labels, pred_labels],
        ['Ground Truth', 'Predizione PointNet'],
    ):
        colors = cmap(labels / (NUM_CLASSES - 1))
        ax.scatter(points_np[:, 0], points_np[:, 1], c=colors[:, :3], s=1, alpha=0.7)
        ax.set_title(title)
        ax.set_xlabel('X (normalizzato)')
        ax.set_ylabel('Y (normalizzato)')
        ax.set_aspect('equal')

    plt.suptitle('Confronto Ground Truth vs Predizione', fontsize=14)
    plt.tight_layout()
    if save_path:
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close(fig)
...
def print_test_results(test_acc, test_miou, test_ious, true_labels, pred_labels):
    """Stampa un riassunto completo dei risultati sul test set."""
